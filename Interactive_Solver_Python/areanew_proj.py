...
def read_riddle(i, shape, num_allowed, tri_or_sq):
    if check_valid(shape, num_allowed):
        if tri_or_sq == 'T':
            return write_model_tri(i, shape, num_allowed)
        elif tri_or_sq == 'F':
...
f.close()'''
    build_time = timeit.timeit(code, number=1)
    run_model('areatri' + str(j) + '.smv', j, len(shape) / 2 + num_allowed)
    return build_time
...
def get_shapes(j, original):
    ...
    return cl1 and cl2, shape1, shape2
    """
        for ang in inside2:
            shape2.append(ang)
        shape1.append(cuts1[0])
...
def solve_rid_input(j, original, num_allowed, tri_or_sq):
    flag_solved = 0
    run_time = -1
    shape1 = None
    shape2 = None
    ...
    if times != -1:
        ...
        if tri_or_sq == 'T':
            ...
            closed = True
            if flag_solved == 2:
                closed, shape1, shape2 = get_shapes(j, original)
            ...
            write_model_sq(j, original, num_m)
            flag_solved, run_time = find_solution(j)
            total += run_time
            closed = True
            if flag_solved == 2:
